...
test.describe('전체 흐름 E2E 테스트', () => {
  ...
  test.describe('4. 게시글 CRUD 테스트', () => {
    ...
    test('게시글 다중 삭제', async ({ page }) => {
      ...
      // 체크박스 선택 (두 게시글 모두)
      // 구조: table > tbody > tr > td(checkbox) + td(id) + td(title) + ...
      // 게시글 제목이 포함된 테이블 행에서 체크박스 찾기
      const row1 = page.locator('tbody tr').filter({ has: page.getByText(title1, { exact: true }) });
      const row2 = page.locator('tbody tr').filter({ has: page.getByText(title2, { exact: true }) });
